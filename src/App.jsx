  ...
  return (
    <main>
      ...
      <section className="services section-shell" id="servicos">
        ...
        <div className="service-grid">{serviceHighlights.map((service, index) => <a className="service-card" key={service.slug} href={service.href} aria-label={`Conhecer ${service.cardTitle}`} data-cta-source="service-highlight" data-cta-service={service.cardTitle} data-service-slug={service.slug}><img src={service.image} alt="" loading="lazy" /><span className="service-card__number">0{index + 1}</span><h3>{service.cardTitle}</h3></a>)}</div>
      </section>
